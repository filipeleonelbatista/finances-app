...
export default function IAPage() {
  ...
  const { checkIfApiKeyExists, openaiErrorHandler, handleSaveApiKey, apiKey } =
    useOpenAi();
  ...
  const financesList = useMemo(() => {
    const headers =
      "valor;vencimento;data de vencimento;data de pagamento;descrição;categorias;pago";
  ...
  const fuelCostsList = useMemo(() => {
    const headers =
      "valor pago;valor do litro do combustível;data de abastecimento;local do abastecimento;tipo de combustivel;Kilometragem do veiculo no abastecimento";
    ...
    try {
      ...
      const result = await openai.createChatCompletion({
        ...
        messages: [
          {
            role: "user",
            content: userMessage.replaceAll("                ", ""),
          },
        ],
        temperature: 1,
        max_tokens: 2048, //256,
      ...
      setData([
        ...
        {
          type: "user",
          text: "De acordo com os gastos que tive esse mês, quais gastos poderiam ser melhorados?",
        },
  ...
  const handleAskForFuelCostsTips = async () => {
    try {
      ...
      setData([
        ...
        {
          type: "user",
          text: "De acordo com os abastecimentos que tive esse mês, o que posso melhorar para evitar gastos?",
        },
        { type: "assistant", text: result.data.choices[0].message.content },
      ]);
  ...
  const handleAskForMarketCostsTips = async () => {
    try {
      ...
      setData([
        ...
        {
          type: "user",
          text: "De acordo com as compras de mercado que tive esse mês, quais gastos poderiam ser melhorados?",
        },
        { type: "assistant", text: result.data.choices[0].message.content },
      ]);
  ...
  const handleAskToAi = async () => {
    try {
      ...
      const newDataForConversation = data.map((item) => ({
        role: item.type,
        content: item.text,
      }));
        ...
        >
          {data.length === 0 && !isLoading ? (
            ...
            >
              ...
              >
                <HStack position={"relative"} alignItems="center" space={4}>
              ...
              >
                <HStack position={"relative"} alignItems="center" space={4}>
                  ...
                  <Text color={text} maxW={"90%"}>
                    Analise meus gastos com mercado e sugira pontos onde eu
                    posso melhorar a alimentação.
                  </Text>
                </HStack>
              </Pressable>
            ...
            >
              {data.map((mensagem, index) => (
                <HStack
                  ...
                  px={4}
                  flexDirection={
                    mensagem.type === "user" ? "row" : "row-reverse"
                  }
                ...
                >
                  ...
                  >
                    <MaterialCommunityIcons
                      name={
                        mensagem.type === "user" ? "account" : "robot-outline"
                      }
                      size={20}
                      color={headerText}
                    />
        ...
        >
          ...
          <HStack space={2}>
            ...
            <IconButton
              ...
              icon={
                <Octicons name="paper-airplane" size={20} color={headerText} />
              }
              _pressed={{
                bgColor: "purple.900",
              }}
      ...
      >
        <Actionsheet.Content pb={isKeyboardOpen ? 24 : 0}>
          <VStack space={4} p={4}>
            ...
            <VStack space={2}>
              <Text color={text}>Cole sua chave de api aqui.</Text>
              <Input
