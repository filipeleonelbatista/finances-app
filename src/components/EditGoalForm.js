...
export default function EditGoalForm({ onClose, selectedTransaction }) {
  ...
  async function handleSubmitForm(formValues) {
    ...
    const data = {
      ...
      currentAmount: parseFloat(
        formValues.currentAmount.replaceAll(".", "").replace(",", ".")
      ),
      amount: parseFloat(
  ...
  return (
    <ScrollView w={"100%"} px={4}>
      ...
      <VStack space={2}>
        <Text color={text}>Valor da meta</Text>
        <Input
          editable={isEditable}
          isDisabled={!isEditable}
      ...
      <VStack space={2}>
        <Text color={text}>Valor alcançado</Text>
        <Input
          editable={isEditable}
          isDisabled={!isEditable}
          ...
          onChangeText={(text) =>
            formik.setFieldValue("currentAmount", moeda(text))
          }
      ...
      <VStack space={2}>
        <Text color={text}>Data da meta</Text>
        <Input
          isDisabled={!isEditable}
